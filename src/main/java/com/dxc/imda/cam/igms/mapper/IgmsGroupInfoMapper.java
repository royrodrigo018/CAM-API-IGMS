...
@Component
public class IgmsGroupInfoMapper {
	...
	public UpdateInfo convertUpdateInfoToJSON(UserRole userRole) {
		...
		try {
			...
			updateInfo.setUserId(userRole.getGroupUserMapList().get(0).getUserProfile().getUserId());
	...
	private Meta getMeta(UserRole userRole) {
		...
		//logger.info("getMeta userRole.getGroupUserMapList().size(): " + userRole.getGroupUserMapList().size());
	...
	private List<Member> getMemberList(UserRole userRole){
		...
		//logger.info("userRole.getGroupUserMapList(): " + userRole.getGroupUserMapList().size());
		for(GroupUserMap groupUserMap: userRole.getGroupUserMapList()) {
			//logger.info("groupUserMap.getUserProfile(): " + groupUserMap.getUserProfile());
			//logger.info("groupUserMap.getUserProfile().getStatus(): " + groupUserMap.getUserProfile().getStatus());
			if ("Active".equalsIgnoreCase(groupUserMap.getUserProfile().getStatus())){
				Member member = new Member();
				member.setValue(groupUserMap.getUserProfile().getUserId());
				member.set$ref("");
				member.setType("User");
				members.add(member);
			}
	...
	private ExtensionCamGroup getExtensionCamGroup(UserRole userRole) {
		...
		logger.info("userRole.getUserRoleAccesses(): " + userRole.getGroupRoleMapList());
		for (GroupRoleMap groupRoleMap: userRole.getGroupRoleMapList()) {
			sb.append(groupRoleMap.getGroupRole().getGroupRoleDesc());
			sb.append(", ");
		}
